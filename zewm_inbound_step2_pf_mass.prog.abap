...
*&---------------------------------------------------------------------*
REPORT zewm_inbound_step2_pf_mass.
...
DATA lt_message     TYPE /scdl/dm_message_tab.
...
PARAMETERS:
  ...
  p_ibditm TYPE /scdl/dl_itemno      NO-DISPLAY,
  p_qty    TYPE /scdl/dl_quantity    NO-DISPLAY,
...
*--------------------------------------------------------------------------
  IF lv_error = abap_false.
    LOOP AT lt_pdi_itm ASSIGNING FIELD-SYMBOL(<ls_item>).
      PERFORM create_whr
        USING    p_lgnum
                 p_ws
                 p_qty
  ...
  ELSE.
    LOOP AT lt_message INTO DATA(ls_msg).
      WRITE :/ ls_msg-msgty, ls_msg-msgid, ls_msg-msgno, ls_msg-msgv1, ls_msg-msgv2, ls_msg-msgv3, ls_msg-msgv4 .
    ENDLOOP.
...
FORM query_pdi
  ...
  CHANGING ct_itm         TYPE /scwm/dlv_item_out_prd_tab
           ...
           ct_message     TYPE /scdl/dm_message_tab.
...
* Destination HU
  ...
  ls_create_whr-anfme   = is_item-qty-qty.
  ...
  PERFORM convert_bapiret USING lt_bapiret
                       CHANGING ct_message.
...
FORM convert_bapiret USING it_bapiret   TYPE bapirettab
                  CHANGING ct_messsage  TYPE /scdl/dm_message_tab.

  DATA ls_msg TYPE /scdl/dm_message_str.
  LOOP AT it_bapiret INTO DATA(ls_ret).
